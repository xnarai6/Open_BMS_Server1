const CMDVal = {
    ...
    RESIP: Buffer.from([0x0d, 0xa1]),
    RESPERIOD: Buffer.from([0x0d, 0xa2]),
};
...
const CMDBizData = {
    IBT: Buffer.from([
        0x3a, // STX              : 0x3A
        0x30,
        0x30, // ADDRESS          : 0(int) -> 0(hex) -> 00(byte) -> 0x30 0x30(uft8 buffer)
        0x30,
        0x32, // CMD              : 2(int) -> 2(hex) -> 02(byte) -> 0x30 0x32(uft8 buffer)
        0x30,
        0x30, // VERSION          : 0(int) -> 0(hex) -> 00(byte) -> 0x30 0x30(uft8 buffer)
        0x30,
        0x30,
        0x34,
        0x34, // LEN              : 68(int) -> 44(hex) -> 0044(byte) -> 0x30 0x30 0x34 0x34(uft8 buffer)
        0x30,
        0x32, // BAT SERIAL NUM   : 2(int) -> 2(hex) -> 02(byte) -> 0x30 0x32(uft8 buffer)
        0x41,
        0x30,
        0x34,
        0x33, // VOLT             : 41027(int) -> A043(hex) -> A043(byte) -> 0x41 0x30 0x34 0x33(uft8 buffer)
        0x41,
        0x30,
        0x34,
        0x31, // VOLT             : 41025(int) -> A041(hex) -> A041(byte) -> 0x41 0x30 0x34 0x31(uft8 buffer)
        0x30,
        0x31,
        0x41,
        0x35, // CURR1            : 421(int) -> 1A5(hex) -> 01A5(byte) -> 0x30 0x31 0x41 0x35(uft8 buffer)
        0x30,
        0x30,
        0x30,
        0x30, // CURR2            : 0(int) -> 0(hex) -> 0000(byte) -> 0x30 0x30 0x30 0x30(uft8 buffer)
        0x30,
        0x31, // SYS TP NUM       : 1(int) -> 1(hex) -> 0001(byte) -> 0x30 0x30 0x30 0x31(uft8 buffer)
        0x33,
        0x34, // SYS TP           : 52(int) -> 34(hex) -> 34(byte) -> 0x33 0x34(uft8 buffer)
        0x30,
        0x31, // FET TP NUM       : 1(int) -> 1(hex) -> 0001(byte) -> 0x30 0x30 0x30 0x31(uft8 buffer)
        0x33,
        0x32, // FET TP           : 50(int) -> 32(hex) -> 32(byte) -> 0x33 0x32(uft8 buffer)
        0x30,
        0x31, // LTC TP NUM       : 1(int) -> 1(hex) -> 0001(byte) -> 0x30 0x30 0x30 0x31(uft8 buffer)
        0x32,
        0x38, // LTC TP           : 40(int) -> 28(hex) -> 28(byte) -> 0x32 0x38(uft8 buffer)
        0x30,
        0x31, // CELL TP NUM      : 1(int) -> 1(hex) -> 0001(byte) -> 0x30 0x30 0x30 0x31(uft8 buffer)
        0x34,
        0x31, // CELL TP          : 65(int) -> 41(hex) -> 41(byte) -> 0x34 0x31(uft8 buffer)
        0x30,
        0x31,
        0x30,
        0x31, // ALARM STAT       :
    ...
    TS: Buffer.from([
        0x2, 0x51, 0x10, 0x83, 0x0, 0x0, 0x97, 0x1, 0x0, 0xb, 0x0, 0x1, 0x0, 0x10, 0x83, 0x3, 0x2, 0x52, 0x10, 0x83, 0x0, 0x0, 0x1, 0x10, 0x82, 0x0, 0x0, 0x10, 0x8f, 0xe, 0x8, 0x7, 0x3, 0x2, 0x53, 0x10, 0x83, 0x0, 0x0, 0x56, 0xe, 0x55, 0xe, 0x55, 0xe, 0x0, 0x10, 0x82, 0x3, 0x2, 0x54, 0x10, 0x83, 0x0, 0x0, 0x1a, 0x1a, 0x1a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2,
        0x55, 0x10, 0x83, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2, 0x58, 0x10, 0x83, 0x0, 0x0, 0x0, 0x55, 0xe, 0x1e, 0xa, 0x1, 0x0, 0x0, 0x3, 0x2, 0x56, 0x10, 0x83, 0x0, 0x0, 0xc1, 0x81, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2, 0x57, 0x10, 0x83, 0x0, 0x0, 0x61, 0xfe, 0xff, 0xff, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2, 0x59, 0x10, 0x83, 0x0, 0x0,
        0x8c, 0xd7, 0x82, 0x1, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2, 0x5f, 0x10, 0x83, 0x0, 0x0, 0x98, 0x1, 0xca, 0x5, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2, 0x0, 0x1, 0x0, 0x0, 0x56, 0xe, 0x56, 0xe, 0x55, 0xe, 0x56, 0xe, 0x3, 0x2, 0x11, 0x1, 0x0, 0x0, 0x56, 0xe, 0x55, 0xe, 0x53, 0xe, 0x0, 0x0, 0x3, 0x2, 0x22, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2,
        0x6, 0x1, 0x0, 0x0, 0x1a, 0x1a, 0x1a, 0x1a, 0x0, 0x0, 0x0, 0x0, 0x3, 0x2, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x3,
    ]),
};

// make head
// STX + SOH + LEN + CMD + BRD
// LEN: CMD(2) + BRD(24) + DATA(?) + CRC(1) + ETX(1)
function makeHead(type, boardnum, dataLen) {
    const totalLen = 2 + 24 + dataLen + 1 + 1;
    const lenBuff = Buffer.alloc(2);
    lenBuff.writeIntBE(totalLen, 0, 2);

    const STX = Buffer.from([0x02]);
    const SOH = Buffer.from([0x01]);
    const LEN = lenBuff;
    const CMD = CMDVal[type];
    const BOARDNUM = Buffer.from(boardnum, 'hex');
...
function makeData(type) {
    if (type == 'TH') return Buffer.concat([makeRandomBuff(-500, 4000, 2), makeRandomBuff(0, 10000, 2), makeRandomBuff(-500, 4000, 2), makeRandomBuff(0, 10000, 2)]);
    if (type == 'VC') return Buffer.concat([makeRandomBuff(0, 100000, 4), makeRandomBuff(0, 100000, 4)]);
    if (type == 'LOC') return Buffer.concat([Buffer.from([37]), makeRandomBuff(5110000, 5119999, 4), Buffer.from([127]), makeRandomBuff(270000, 279999, 4)]);

    if (type == 'IBT' || type == 'TS') return makeBizData(type);
    if (type == 'RESIP' || type == 'RESPERIOD') return makeResData(type);
}

function makeBizData(type) {
    return CMDBizData[type] ? CMDBizData[type] : Buffer.from([]);
}

function makeResData(type) {
    return Buffer.from([]);
}
